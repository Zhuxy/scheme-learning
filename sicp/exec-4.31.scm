  ...
  (cond 
      ((memo-trunk? obj)
        (let ((result (actual-value (trunk-exp obj) (trunk-env obj))))
          ...
          (set-cdr! (cdr obj) '())
          result))
      ((trunk? obj) (actual-value (trunk-exp obj) (trunk-env obj)))
...
(define (memo-trunk? obj) (tagged-list? obj 'memo-trunk))
...
(define (list-of-delayed-args parameters exps env)
  (if (no-operands? exps) '()
    (cons (trunk-or-memo-trunk (car parameters) (first-operand exps) env)
      (list-of-delayed-args (cdr parameters) (rest-operands exps) env))))

(define (trunk-or-memo-trunk parameter exp env)
  (cond ((symbol? parameter) (actual-value exp env))
    ((eq? (cadr parameter) 'lazy) (list 'trunk exp env))
    ((eq? (cadr parameter) 'lazy-memo) (list 'memo-trunk exp env))
    (else (error 'trunk-or-memo-trunk "wrong parameter format" parameter))))
  ...
  (let ((args (cadr p)))
    (set! args
      (map
        (lambda (a)
          (cond ((symbol? a) a)
            ((eq? (cadr a) 'lazy) (car a))
            ((eq? (cadr a) 'lazy-memo) (car a))
            (else (error 'procedure-parameters "wrong parameters format" a))))
        args))
    args))
...
;4.31
(eval '
  (define (f a (b lazy) c (d lazy-memo)) (+ c d))
  the-global-environment)

(eval '
  (display (f 1 (begin (display "delayed 2") (newline) 2) 3 4))
the-global-environment)
